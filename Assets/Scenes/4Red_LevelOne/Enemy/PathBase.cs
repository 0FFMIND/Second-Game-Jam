...
public class PathBase
{
    ...
    List<Vector2> points;
    [SerializeField, HideInInspector]
    bool isClosed; // 暴露出来给到到Editor
    [SerializeField, HideInInspector]
    bool autoSetControlPoints; // 暴露出来给到Editor

    public PathBase(Vector2 center)
    {
    ...
    public bool IsClosed // 写属性暴露出来给外面访问
    {
        get
        {
            return isClosed;
    ...
    public bool AutoSetControlPoints // 写属性暴露出来给外面访问
    {
        get
        {
    ...
    private Vector2 EvaluateCubic(Vector2 a, Vector2 b, Vector2 c, Vector2 d, float t)
    {
        ...
        return Vector2.Lerp(p0, p1, t); // Lerp是让一点到另一点速度降低的方法，相当于缓出
    }
    // 而绘制三次贝塞尔曲线的方法是将三次的abc，bcd分开，分别绘制出二次贝塞尔曲线
    // 同理二次降下来用Unity数学函数Vector.Lerp绘制差值曲线
